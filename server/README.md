...
### Browse guests / vendors / payments / budget / seating / contracts / timeline / packets / rentals / catering rentals / party / tasks / print overrides — desktop pgAdmin → `127.0.0.1:15432` (db-proxy)
...
Save, then expand **Servers → … → Databases → covenant → Schemas → public → Tables → guests** (or **vendors** / **payments** / **budget_categories** / **seating_tables** / **contracts** / **timeline_events** / **packets** / **rentals** / **catering_rentals** / **party_members** / **planning_tasks** / **vendor_arrivals** / **weddings** for `packet_overrides_json`).
...
### Browse guests / vendors / payments / budget / seating / contracts / timeline / packets / rentals / catering rentals / party / tasks / print overrides — browser pgAdmin (Docker network)
...
Expand **Servers → Covenant Postgres → Databases → covenant → Schemas → public → Tables → guests** (or **vendors** / **payments** / **budget_categories** / **seating_tables** / **contracts** / **timeline_events** / **packets** / **rentals** / **catering_rentals** / **party_members** / **planning_tasks** / **vendor_arrivals** / **weddings** for `packet_overrides_json`).
...
| GET | `/weddings/:id/catering-rentals` | List catering rentals |
| PUT | `/weddings/:id/catering-rentals/:rentalId` | Upsert one (LWW) |
| POST | `/weddings/:id/catering-rentals/bulk` | Upsert many |
| DELETE | `/weddings/:id/catering-rentals/:rentalId` | Delete |
| GET | `/weddings/:id/party` | List wedding party members |
| PUT | `/weddings/:id/party/:memberId` | Upsert one (LWW) |
| POST | `/weddings/:id/party/bulk` | Upsert many |
| DELETE | `/weddings/:id/party/:memberId` | Delete |
| GET | `/weddings/:id/tasks` | List planning tasks |
...
| GET | `/weddings/:id/packet-overrides` | Get print packet overrides blob |
| PUT | `/weddings/:id/packet-overrides` | Upsert blob (LWW on updated_at) |
...
Open **Settings → Cloud sync (beta)** to sign in, upload this wedding, and sync guests + vendors + payments + budget + seating + contracts + timeline + packets + rentals + catering rentals + party + tasks + vendor arrivals + print packet overrides.

## Second-device sync (manual + automated)

Guests, vendors, payments, budget, seating, contracts, timeline events, packets, rentals, catering rentals, party, planning tasks, vendor arrivals, and print packet overrides uploaded on one browser profile must appear on another after the same account signs in and syncs.
...
## Verify guests / vendors / payments / budget / seating / contracts / timeline / packets / rentals / catering rentals / party / tasks / vtimeline / print packet overrides landed in Postgres (Windows)
...
docker exec -it covenant-postgres psql -U covenant -d covenant -c "SELECT id, wedding_id, item, material, color, qty, vendor, source, cost, status, notes, updated_at FROM catering_rentals ORDER BY updated_at DESC LIMIT 50;"
...
docker exec -it covenant-postgres psql -U covenant -d covenant -c "SELECT id, packet_overrides_updated_at, packet_overrides_json FROM weddings ORDER BY packet_overrides_updated_at DESC NULLS LAST LIMIT 20;"
```
...
docker exec -it covenant-postgres psql -U covenant -d covenant -c "SELECT wedding_id, COUNT(*) AS catering_rentals FROM catering_rentals GROUP BY wedding_id;"
...
curl.exe -s -H "Authorization: Bearer $token" "http://127.0.0.1:18787/weddings/$wid/catering-rentals"
...
curl.exe -s -H "Authorization: Bearer $token" "http://127.0.0.1:18787/weddings/$wid/photos"
curl.exe -s -H "Authorization: Bearer $token" "http://127.0.0.1:18787/weddings/$wid/photos/storage"
```
...
3. Backup + photos foundation shipped — [`docs/BACKUP_AND_PHOTOS.md`](../docs/BACKUP_AND_PHOTOS.md). Next: RSVP + gated guest portal — [`docs/PRODUCT_ROADMAP.md`](../docs/PRODUCT_ROADMAP.md).

## Conflict policy

Last-write-wins using guest/vendor/payment/budget/seating/contract/timeline/packet/rental/catering-rental/party/task/vtimeline `updated_at`, plus wedding-scoped blob stamps for floor fixtures and print packet overrides. Server only overwrites when the incoming timestamp is ≥ stored. Client prefers local-newer rows and ACK timestamps from the API. See the architecture doc for details.
