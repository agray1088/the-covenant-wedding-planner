...
Offline-first planner stays the default. This Node + Postgres API is **opt-in** for multi-device guest + vendor + payment + budget + seating + contract sync (beta).
...
### Browse guests / vendors / payments / budget / seating / contracts — desktop pgAdmin → `127.0.0.1:15432` (db-proxy)
...
Save, then expand **Servers → … → Databases → covenant → Schemas → public → Tables → guests** (or **vendors** / **payments** / **budget_categories** / **seating_tables** / **contracts**).
...
### Browse guests / vendors / payments / budget / seating / contracts — browser pgAdmin (Docker network)
...
Expand **Servers → Covenant Postgres → Databases → covenant → Schemas → public → Tables → guests** (or **vendors** / **payments** / **budget_categories** / **seating_tables** / **contracts**).
...
Open **Settings → Cloud sync (beta)** to sign in, upload this wedding, and sync guests + vendors + payments + budget + seating + contracts.

## Second-device sync (manual + automated)

Guests, vendors, payments, budget, seating, and contracts uploaded on one browser profile must appear on another after the same account signs in and syncs.
...
## Verify guests / vendors / payments / budget / seating / contracts landed in Postgres (Windows)
...
6. Expand sync beyond guests + vendors + payments + budget + seating + contracts (timeline, packets, rentals, …) per `docs/OFFLINE_CLOUD_SYNC.md`.

## Conflict policy

Last-write-wins using guest/vendor/payment/budget/seating/contract `updated_at`. Server only overwrites when the incoming timestamp is ≥ stored. Client prefers local-newer rows and ACK timestamps from the API. See the architecture doc for details.
