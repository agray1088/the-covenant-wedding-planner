    ...
    if (s.client_key) {
      ...
      if (existing.rows[0]) {
        const mem = await query(
          `SELECT role, status FROM memberships WHERE wedding_id = $1 AND user_id = $2`,
          [existing.rows[0].id, req.user.id]
        );
        if (!mem.rows[0]) {
          await query(
            `INSERT INTO memberships (wedding_id, user_id, role, status, accepted_at)
             VALUES ($1, $2, 'owner', 'accepted', now())
             ON CONFLICT (wedding_id, user_id) DO NOTHING`,
            [existing.rows[0].id, req.user.id]
          );
        } else if (mem.rows[0].status !== 'accepted') {
          await query(
            `UPDATE memberships
                SET status = 'accepted', role = 'owner', accepted_at = COALESCE(accepted_at, now()),
                    revoked_at = NULL
