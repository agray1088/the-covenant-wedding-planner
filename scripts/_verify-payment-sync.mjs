/**
 * Second-device cloud sync proof (payments vertical).
 *
 * Two isolated Playwright storage contexts = two “devices”.
 * Device A: enable cloud → login demo → upload wedding → add unique payment → sync
 * Device B: fresh context → enable cloud → same login → sync/pull → assert payment present
 *
 ...
 *
 * Or:
 *   node scripts/_verify-payment-sync.mjs
 *   COVENANT_CLOUD_API=http://127.0.0.1:18787 node scripts/_verify-payment-sync.mjs
 *
 * After schema changes (new payments table): docker compose down -v && docker compose up -d
 */
...
const PORT = Number(process.env.E2E_PAYMENT_SYNC_PORT || 8795);
...
async function openDevice(browser, label) {
  ...
  await page.goto(`http://127.0.0.1:${PORT}/?v=payment-sync-${label}`, { waitUntil: 'domcontentloaded' });
...
async function uploadAndSyncPayment(page, paymentDesc) {
  return page.evaluate(async (desc) => {
    if (!Array.isArray(window.data.payments)) window.data.payments = [];
    const id = 'p_2dev_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 8);
    const updatedAt = new Date().toISOString();
    window.data.payments.push({
      id,
      _id: id,
      vendor: 'Second Device Florist',
      desc,
      due: 750,
      paid: 250,
      gratuity: 0,
      gratuityStatus: 'Not Planned',
      date: '2026-06-01',
      paiddate: '',
      ptype: 'Zelle',
      status: 'Partially Paid',
      notes: 'Payment sync proof',
      budgetCat: 'Florals',
      budgetItem: '',
      budgetItemId: '',
      contractIdx: '',
    ...
    return {
      ...
      localPaymentCount: (window.data.payments || []).length,
      pullPayments: sync && sync.pull && sync.pull.payments ? sync.pull.payments.pulled : null
    };
  }, paymentDesc);
}
...
async function syncPullOnDeviceB(page) {
  return page.evaluate(async () => {
    ...
    const sync = await window.CovenantCloudSync.syncNow();
    const payments = (window.data && window.data.payments) || [];
    return {
      weddingIdBefore: before,
      weddingIdAfter: localStorage.getItem('covenant_cloud_wedding_id'),
      status: window.CovenantCloudSync.getStatus(),
      pulledPayments: sync && sync.pull && sync.pull.payments ? sync.pull.payments.pulled : null,
      paymentDescs: payments.map((p) => p && p.desc).filter(Boolean),
      paymentCount: payments.length,
      installmentCounts: payments.map((p) => (p && Array.isArray(p.installments) ? p.installments.length : 0)),
      sync
    };
  });
}
...
function marker() {
  return `SecondDevicePayment-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`;
}

async function main() {
  console.log('payment-sync: API', API);
  await requireApi();

  const server = await startServer();
  const browser = await chromium.launch({ headless: true });
  const paymentDesc = marker();
  let deviceA;
  let deviceB;
  const report = {
    ok: false,
  ...
  try {
    ...
    const aResult = await uploadAndSyncPayment(deviceA.page, paymentDesc);
    ...
    const apiList = await fetch(API + '/weddings/' + aResult.weddingId + '/payments', {
      headers: { Authorization: 'Bearer ' + tokenA }
    }).then((r) => r.json());
    const onServer = Array.isArray(apiList.payments) && apiList.payments.some((p) => p.desc === paymentDesc);
    if (!onServer) {
      report.errors.push('Device A: payment not present on API after sync — ' + JSON.stringify(apiList).slice(0, 500));
    } else {
      const row = apiList.payments.find((p) => p.desc === paymentDesc);
      if (!row.installments || row.installments.length < 2) {
        report.errors.push('Device A: payment on API missing installments');
      }
    }
    ...
    if (!bResult.paymentDescs.includes(paymentDesc)) {
      report.errors.push(
        `Device B missing payment "${paymentDesc}". Have: ${JSON.stringify(bResult.paymentDescs.slice(0, 20))}`
      );
    }
    const pulledRow = await deviceB.page.evaluate((desc) => {
      const list = (window.data && window.data.payments) || [];
      return list.find((p) => p && p.desc === desc) || null;
    }, paymentDesc);
    if (pulledRow && (!Array.isArray(pulledRow.installments) || pulledRow.installments.length < 2)) {
      report.errors.push('Device B: payment pulled without expected installments');
    }
    ...
    if (!report.ok) {
      ...
      console.log('payment-sync ok');
      console.log(JSON.stringify({
        ok: true,
        paymentDesc,
        weddingId: aResult.weddingId,
        deviceAPayments: aResult.localPaymentCount,
        deviceBPulledPayments: bResult.pulledPayments,
        deviceBPayments: bResult.paymentCount,
        installments: pulledRow && pulledRow.installments ? pulledRow.installments.length : null
      }, null, 2));
    }
  } catch (e) {
    console.error('payment-sync FAILED:', e && e.message ? e.message : e);
    process.exitCode = 1;
  } finally {
