/**
 * Second-device cloud sync proof (catering rentals vertical).
 *
 * Distinct from finances rentals (data.rentals) — this covers data.cateringRentals
 * on Catering & Menu (tableware / equipment).
 *
 * Two isolated Playwright storage contexts = two “devices”.
 * Device A: enable cloud → login demo → upload wedding → add unique catering rental → sync
 * Device B: fresh context → enable cloud → same login → sync/pull → assert catering rental present
 *
 ...
 *
 * Or:
 *   node scripts/_verify-catering-rental-sync.mjs
 *   COVENANT_CLOUD_API=http://127.0.0.1:18787 node scripts/_verify-catering-rental-sync.mjs
 *
 * After schema changes (new catering_rentals table): docker compose down -v && docker compose up -d
 */
...
const PORT = Number(process.env.E2E_CATERING_RENTAL_SYNC_PORT || 8802);
...
async function openDevice(browser, label) {
  ...
  await page.goto(`http://127.0.0.1:${PORT}/?v=catering-rental-sync-${label}`, { waitUntil: 'domcontentloaded' });
...
async function uploadAndSyncCateringRental(page, rentalItem) {
  return page.evaluate(async (item) => {
    if (!Array.isArray(window.data.cateringRentals)) window.data.cateringRentals = [];
    const id = 'crt_2dev_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 8);
    const updatedAt = new Date().toISOString();
    window.data.cateringRentals.push({
      id,
      _id: id,
      item,
      material: 'Porcelain',
      ...
      cost: 240,
      status: 'Rented',
      notes: 'Dinner plates — charger set',
      updatedAt
    });
    save();
    ...
    return {
      ...
      localRentalCount: (window.data.cateringRentals || []).length,
      pullCateringRentals: sync && sync.pull && sync.pull.cateringRentals
        ? sync.pull.cateringRentals.pulled
        : null
    };
  }, rentalItem);
}
...
async function syncPullOnDeviceB(page) {
  return page.evaluate(async () => {
    ...
    const sync = await window.CovenantCloudSync.syncNow();
    const rentals = (window.data && window.data.cateringRentals) || [];
    return {
      weddingIdBefore: before,
      weddingIdAfter: localStorage.getItem('covenant_cloud_wedding_id'),
      status: window.CovenantCloudSync.getStatus(),
      pulledRentals: sync && sync.pull && sync.pull.cateringRentals
        ? sync.pull.cateringRentals.pulled
        : null,
      rentalItems: rentals.map((r) => r && r.item).filter(Boolean),
...
function marker() {
  return `SecondDeviceCateringRental-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`;
}

async function main() {
  console.log('catering-rental-sync: API', API);
  await requireApi();

  const server = await startServer();
  const browser = await chromium.launch({ headless: true });
  const rentalItem = marker();
  let deviceA;
  let deviceB;
  const report = {
    ok: false,
  ...
  try {
    ...
    const aResult = await uploadAndSyncCateringRental(deviceA.page, rentalItem);
    ...
    const apiList = await fetch(API + '/weddings/' + aResult.weddingId + '/catering-rentals', {
      headers: { Authorization: 'Bearer ' + tokenA }
    }).then((r) => r.json());
    const onServer = Array.isArray(apiList.cateringRentals)
      && apiList.cateringRentals.some((r) => r.item === rentalItem);
    if (!onServer) {
      report.errors.push('Device A: catering rental not present on API after sync — ' + JSON.stringify(apiList).slice(0, 500));
    } else {
      const row = apiList.cateringRentals.find((r) => r.item === rentalItem);
      if (!row || row.vendor !== 'Catering Co.') {
        report.errors.push('Device A: catering rental on API missing/wrong vendor');
      }
      if (!row || row.material !== 'Porcelain') {
        report.errors.push('Device A: catering rental on API missing/wrong material');
      }
      if (!row || row.color !== 'Ivory') {
        report.errors.push('Device A: catering rental on API missing/wrong color');
      }
      if (!row || String(row.qty) !== '120') {
        report.errors.push('Device A: catering rental on API missing/wrong qty');
      }
      if (!row || row.source !== 'Rented') {
        report.errors.push('Device A: catering rental on API missing/wrong source');
      }
      if (!row || Number(row.cost) !== 240) {
        report.errors.push('Device A: catering rental on API missing/wrong cost');
      }
      if (!row || row.status !== 'Rented') {
        report.errors.push('Device A: catering rental on API missing/wrong status');
      }
      if (!row || row.notes !== 'Dinner plates — charger set') {
        report.errors.push('Device A: catering rental on API missing/wrong notes');
      }
    }
    ...
    if (!bResult.rentalItems.includes(rentalItem)) {
      report.errors.push(
        `Device B missing catering rental "${rentalItem}". Have: ${JSON.stringify(bResult.rentalItems.slice(0, 20))}`
      );
    }
    const pulledRow = await deviceB.page.evaluate((item) => {
      const list = (window.data && window.data.cateringRentals) || [];
      return list.find((r) => r && r.item === item) || null;
    }, rentalItem);
    if (pulledRow && pulledRow.vendor !== 'Catering Co.') {
      report.errors.push('Device B: catering rental pulled with wrong vendor');
    }
    if (pulledRow && pulledRow.material !== 'Porcelain') {
      report.errors.push('Device B: catering rental material not pulled');
    }
    if (pulledRow && pulledRow.source !== 'Rented') {
      report.errors.push('Device B: catering rental source not pulled');
    }
    if (pulledRow && Number(pulledRow.cost) !== 240) {
      report.errors.push('Device B: catering rental cost not pulled');
    }
    ...
    if (!report.ok) {
      console.error('catering-rental-sync FAILED');
      report.errors.forEach((e) => console.error(' -', e));
      console.error(JSON.stringify(report, null, 2));
      process.exitCode = 1;
    } else {
      console.log('catering-rental-sync ok');
      console.log(JSON.stringify({
        ok: true,
        rentalItem,
        weddingId: aResult.weddingId,
        deviceARentals: aResult.localRentalCount,
        deviceBPulledRentals: bResult.pulledRentals,
        deviceBRentals: bResult.rentalCount,
        fields: pulledRow ? {
          vendor: pulledRow.vendor,
          material: pulledRow.material,
          color: pulledRow.color,
          qty: pulledRow.qty,
          source: pulledRow.source,
          cost: pulledRow.cost,
          status: pulledRow.status,
          notes: pulledRow.notes
        } : null
      }, null, 2));
    }
  } catch (e) {
    console.error('catering-rental-sync FAILED:', e && e.message ? e.message : e);
    process.exitCode = 1;
  } finally {
