/**
 * Second-device cloud sync proof (rentals vertical).
 *
 * Two isolated Playwright storage contexts = two “devices”.
 * Device A: enable cloud → login demo → upload wedding → add unique rental → sync
 * Device B: fresh context → enable cloud → same login → sync/pull → assert rental present
 *
 ...
 *
 * Or:
 *   node scripts/_verify-rental-sync.mjs
 *   COVENANT_CLOUD_API=http://127.0.0.1:18787 node scripts/_verify-rental-sync.mjs
 *
 * After schema changes (new rentals table): docker compose down -v && docker compose up -d
 */
...
const PORT = Number(process.env.E2E_RENTAL_SYNC_PORT || 8801);
...
async function openDevice(browser, label) {
  ...
  await page.goto(`http://127.0.0.1:${PORT}/?v=rental-sync-${label}`, { waitUntil: 'domcontentloaded' });
...
async function uploadAndSyncRental(page, rentalItem) {
  return page.evaluate(async (item) => {
    if (!Array.isArray(window.data.rentals)) window.data.rentals = [];
    const id = 'rnt_2dev_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 8);
    const updatedAt = new Date().toISOString();
    window.data.rentals.push({
      id,
      _id: id,
      item,
    ...
    return {
      ...
      localRentalCount: (window.data.rentals || []).length,
      pullRentals: sync && sync.pull && sync.pull.rentals ? sync.pull.rentals.pulled : null
    };
  }, rentalItem);
}
...
async function syncPullOnDeviceB(page) {
  return page.evaluate(async () => {
    ...
    const sync = await window.CovenantCloudSync.syncNow();
    const rentals = (window.data && window.data.rentals) || [];
    return {
      weddingIdBefore: before,
      weddingIdAfter: localStorage.getItem('covenant_cloud_wedding_id'),
      status: window.CovenantCloudSync.getStatus(),
      pulledRentals: sync && sync.pull && sync.pull.rentals ? sync.pull.rentals.pulled : null,
...
function marker() {
  return `SecondDeviceRental-${Date.now()}-${Math.random().toString(16).slice(2, 8)}`;
}

async function main() {
  console.log('rental-sync: API', API);
  await requireApi();
  ...
  try {
    ...
    const aResult = await uploadAndSyncRental(deviceA.page, rentalItem);
    ...
    const apiList = await fetch(API + '/weddings/' + aResult.weddingId + '/rentals', {
      headers: { Authorization: 'Bearer ' + tokenA }
    }).then((r) => r.json());
    const onServer = Array.isArray(apiList.rentals) && apiList.rentals.some((r) => r.item === rentalItem);
    if (!onServer) {
      report.errors.push('Device A: rental not present on API after sync — ' + JSON.stringify(apiList).slice(0, 500));
    } else {
      const row = apiList.rentals.find((r) => r.item === rentalItem);
      if (!row || row.vendor !== 'Event Rentals Co.') {
        report.errors.push('Device A: rental on API missing/wrong vendor');
      }
      if (!row || row.pickup !== '2026-06-05') {
        report.errors.push('Device A: rental on API missing/wrong pickup');
      }
      if (!row || row.ret !== '2026-06-08') {
        report.errors.push('Device A: rental on API missing/wrong return (ret)');
      }
      if (!row || Number(row.cost) !== 840) {
        report.errors.push('Device A: rental on API missing/wrong cost');
      }
      if (!row || row.details !== 'Chiavari chairs — white') {
        report.errors.push('Device A: rental on API missing/wrong details');
      }
    }
    ...
    if (!bResult.rentalItems.includes(rentalItem)) {
      report.errors.push(
        `Device B missing rental "${rentalItem}". Have: ${JSON.stringify(bResult.rentalItems.slice(0, 20))}`
      );
    }
    const pulledRow = await deviceB.page.evaluate((item) => {
      const list = (window.data && window.data.rentals) || [];
      return list.find((r) => r && r.item === item) || null;
    }, rentalItem);
    if (pulledRow && pulledRow.vendor !== 'Event Rentals Co.') {
      report.errors.push('Device B: rental pulled with wrong vendor');
    }
    if (pulledRow && pulledRow.pickup !== '2026-06-05') {
      report.errors.push('Device B: rental pickup not pulled');
    }
    if (pulledRow && pulledRow.ret !== '2026-06-08') {
      report.errors.push('Device B: rental ret not pulled');
    }
    ...
    if (!report.ok) {
      console.error('rental-sync FAILED');
      report.errors.forEach((e) => console.error(' -', e));
      console.error(JSON.stringify(report, null, 2));
      process.exitCode = 1;
    } else {
      console.log('rental-sync ok');
      console.log(JSON.stringify({
        ...
        fields: pulledRow ? {
          ...
          pickup: pulledRow.pickup,
          ret: pulledRow.ret,
          cost: pulledRow.cost,
          details: pulledRow.details
        } : null
      }, null, 2));
    }
  } catch (e) {
    console.error('rental-sync FAILED:', e && e.message ? e.message : e);
    process.exitCode = 1;
  } finally {
