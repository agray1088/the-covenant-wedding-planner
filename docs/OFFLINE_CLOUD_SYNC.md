...
| **Cloud (optional)** | Sign-in links this device’s wedding to Postgres. **Guests + vendors + payments + budget + seating** sync in beta. Second-device pull uses the same account membership. Hosted vendor portal comes later. |

**Hard rule:** never block save, navigate, or guest/vendor/payment/budget/seating edits on being online. If the API is down or cloud is disabled, the planner behaves exactly like offline GA.
...
└──────────────────────┬──────────────────────┘
         ...
         budget_categories · seating_tables
```

### Client responsibilities

- Always persist locally first (`save()` → LS + SQLite write-through).
- When cloud is **enabled + authenticated**, debounce a guests+vendors+payments+budget+seating push/pull after saves.
...
- Accept guest + vendor + payment + budget + seating CRUD for a wedding the caller belongs to.
...
1. Each guest, vendor, payment, budget category, seating table, and wedding carries `updated_at` (ISO-8601).
...
3. **Pull:** client fetches guests, vendors, payments, budget, and seating; if server `updated_at` is newer than local, replace local row; otherwise keep local and schedule a push.
4. **Upload this wedding:** one-shot migration — create (or claim) a cloud wedding, push **all** local guests, vendors, payments, budget categories, and seating tables, store `cloudWeddingId` on the device profile. Does not delete local data.
...
| Role | Access |
|------|--------|
| `owner` | Full wedding + guest/vendor/payment/budget/seating CRUD; invite members |
| `partner` | Full guest/vendor/payment/budget/seating CRUD (same wedding) |
| `planner` | Full guest/vendor/payment/budget/seating CRUD (coordinator) |
...
3. All local `data.guests`, `data.vendors`, `data.payments`, `data.budget`, and `data.tables` upsert into `guests` / `vendors` / `payments` / `budget_categories` / `seating_tables` (plus floor fixtures on the wedding).
4. Device stores `covenant_cloud_wedding_id` (+ session token) in localStorage.
5. Offline editing continues; next **Sync now** (or debounced auto-sync) reconciles all five verticals.
...
**Schema recreate note:** after pulling seating sync, run `docker compose down -v` then `up -d` so Postgres applies the new `seating_tables` table and wedding floor-fixture columns (volume wipe is expected for local Docker Desktop).
...
## Manual test: second device (guests + vendors + payments + budget + seating)

Prove a guest, vendor, payment, **or** budget category added on device A appears on device B after sign-in + sync.
...
npm run verify:budget-sync
npm run verify:seating-sync
# or: node scripts/_verify-seating-sync.mjs
```

Guest verify uses two isolated Playwright storage contexts, demo login `demo@covenant.local` / `covenant-demo`, and asserts the unique guest pulled onto device B. Vendor / payment / budget / seating verifies do the same for a unique vendor name, payment description, budget category name, or table name.
...
2. **Device A** — normal Chrome window:
   ...
   - **Upload this wedding**, then add a uniquely named guest, vendor, payment, budget category, and/or seating table, then **Sync now**.
3. **Device B** — second Chrome profile **or** an Incognito window (separate storage):
   ...
   - The client links the account’s existing cloud wedding, pulls guests + vendors + payments + budget + seating, and the unique name from device A should appear.
4. Optional DB check: see `server/README.md` (pgAdmin / `docker exec` … `SELECT … FROM guests` / `vendors` / `payments` / `budget_categories` / `seating_tables`).

**Pull this branch:** `git pull origin cursor/offline-cloud-sync-017e`

## Out of scope for this foundation pass

- Full table sync (contracts, timeline, packets, party, …)
