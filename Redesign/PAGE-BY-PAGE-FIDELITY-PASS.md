...
- [38 · Database Hub](#section-38) — done on `cursor/dashboard-views-017e` — verified `dh-s38b` — 5 screens · 2 record drawers — Full page · View switcher views · Section tabs · Record drawer tabs · Night theme
...
- **Status:** done on `cursor/dashboard-views-017e` — verified `dh-s38b`; review, then say “next” for §39
- **Screen ids:** `7b`, `7c`, `19o`

> **Verified this pass (`dh-s38b`).** Full §38 inventory vs Master: Documents tab + sub-nav Database Hub (moved out of Planning per 7b); page view switcher **Database Hub · all tables**; 7b overview (backup card, grouped inventory table, Needs attention with inline fixes, Tables/Links/Activity surface switcher, five-stat strip, rail Views + Storage); 7c all-tables browser (guest filter chips + Search N rows, Columns/Auto-fit/Row height, All rows/Schema/SQL, bulk bar, raw grid); 19o twenty-four scrollable table tabs; Hub table drawer (Table · Fields · Links · Activity) + Hub row drawer (Row · Links · Raw · History); night tokens on tabs, backup, attention, drawer, and row selection.
...
- **Status:** done on `cursor/dashboard-views-017e` — review, then say “next” for §40. verified §39 this pass; review, then say “next” for §40. See the implementation note below.
- **Screen ids:** `49d`, `49a`, `49b`, `49c`

> **Implementation note (§39).** This section is cross-cutting chrome, most of it already delivered when the shell was built. Verified already-done: **49d** — the two-row forest top bar built by `redesign-shell.js`; **49a** — Profile & Display drawer at **460px** with forest header, planning-view segmented control, two-up category cards, and **four record-drawer tabs** (Profile · Display · Alerts · Access) via `js/profile-drawer-redesign.js`; **49b** — Theme Builder with Card Corners defaulting to `None (square)` and a **4×3** colour grid with component preview. **49c** — `js/settings-window-redesign.js` now matches the Master card grid: **forest header**, **green save-status banner** (`#EEF6F0` / `#2F6B45`, amber when backup overdue, refreshed on each open), and **six labelled pane cards** in a 3×2 grid; live controls still relocated by id. **Top-bar menus** (help flyout, undo history flyout, search Records·Pages + ⌘K footer) in **`js/chrome-menus-redesign.js`**. Undo/Redo stay honest in the top-bar undo slot. Eleven Settings standalone screens and Task/Vendor/Payment record editors remain existing surfaces. Cache-bust: `s39-chrome` / `settings-49c-grid` / `s39-menus`.
