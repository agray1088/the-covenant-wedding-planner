...
- [39 · App chrome, settings & record editor](#section-39) — done on `cursor/dashboard-views-017e` — verified `s39-chrome-b` — 8 screens · 5 record drawers — Full page · Record drawer tabs · Night theme
- [40 · Vendor Portal](#section-40) — done on `cursor/dashboard-views-017e` — verified `vp2-s40d` — 7 screens — Full page
- [41 · House style & visual directions](#section-41) — realized by the redesign on `cursor/dashboard-views-017e` — verified `hs-s41a` — 11 screens — Full page · View switcher views · Night theme
- [42 · System-wide patterns](#section-42) — done on `cursor/dashboard-views-017e` — verified `furniture-s42b` — 40 screens — Full page · View switcher views · Night theme
...
- **Status:** done on `cursor/dashboard-views-017e` — verified `s39-chrome-b`; review, then say “next” for §40. See the implementation note below.
- **Screen ids:** `49d`, `49a`, `49b`, `49c`

> **Implementation note (§39 · `s39-chrome-b`).** Cross-cutting chrome. **49d** two-row forest top bar (`redesign-shell.js`). **49a** Profile & Display at 460px with four tabs Profile · Display · Alerts · Access — Access tab now includes Master **Can reach / Cannot** summary (additive; Preview Mode / roles kept). **49b** Theme Builder 4×3 + square Card Corners default. **49c** Settings: forest header, green banner, **six Overview cards + Planner alert rules kept**, plus Master **eleven-pane sidebar** (This device / The planner / This file / Help) that deep-links without removing the card grid. Top-bar menus in `chrome-menus-redesign.js`. Product additions preserved (alert rules, Hub Fields, etc.). Cache-bust: `s39-chrome-b` / `settings-49c-panes` / `pd-49a7` / `s39-menus`.
...
- **Status:** done on `cursor/dashboard-views-017e` — verified `vp2-s40d`; review, then say “next” for §41. See the implementation note below.

> **Implementation note (§40).** The Vendor Portal is a self-contained product — `vendor-portal.html` + `css/vendor-portal.css` + `js/vendor-portal.js`, reached from Share Packets / roles via a tokened `?g=` link. Five of the seven drawn screens were already built: **Your brief**, **Your schedule**, **Your paperwork**, **Upload** (the four vendor tabs), and **Mobile, and the expired link** (the `@media (max-width:520px)` layout with short tab labels + the calm `renderExpired` “This link has expired” state on `?expired=1`). This pass added the two “rules underneath” screens the drawing calls out as *the ones an implementer needs* — **The scope contract** (the 15-row Data · Vendor · Couple · Planner · Why security-model table, marks and load-bearing reasons verbatim from the Master, incl. the `counts` / `2 numbers` / `granted` / `own` special values) and **Access lifecycle** (the five steps packet→expiry plus the honest “what revocation can and cannot do” table). They live behind a footer link (“Why you can see this — the scope & lifecycle behind this link”) as an overlay, not a fifth vendor tab, because *the four tabs above are what these two produce*; the portal is their intended home since revocation honesty is drawn as stated “in the dialog **and here**.” Counts-without-names holds throughout — no guest identity is ever surfaced. **V2 schedule fidelity** — `renderSchedule` now draws the Gantt timeline (time axis, positioned hatched load-in/clear-down bars and solid service bars, stats row, mock footnote, “Accept this schedule”). **V5 mobile fidelity** — at ≤520px the Brief tab switches to the stacked mobile layout (next obligation, overdue upload CTA, Call rows). **`vp2-s40d` Master copy pass (additive only)** — Paperwork pagehead matches Master (*vendor name* + “Your contract and your invoices — no other vendor’s”; Contract value / Next due `1 Nov`; Service agreement with `signed 2 April · 6 pages`; instalments “not typed”; closing line “cannot be added to it”). Expired state uses Master body copy (“Access ran to … this only closes the live view”) with primary **Request access from Mary Osei**, keeping Message as a secondary action. Provenance banner dates as day+month without year (`12 November`). Lifecycle step 2 states the caterer-not-a-user line. Brief clear-down row and dual expired CTAs kept as product additions. Cache-bust: `vp2-s40d`.
...
- **Status:** realized by the redesign on `cursor/dashboard-views-017e` — verified `hs-s41a`; review, then say “next” for §42. No new surface: see the note below.
- **Screen ids:** `2a`, `2b`, `1a`, `1b`, `1c`, `1d`

> **Implementation note (§41).** This section is a **design-decision record**, not a shippable page — its screen headers carry no build notes, and the Master frames it as “Do not invent UX.” The full-page drawings `2a` (Neutral Records × Editorial chrome) and `2b` (its night mode) are renderings of the *shipped* house style; the view-switcher drawings `1a` Baseline / `1b` Direction A — Neutral Records / `1c` Direction B — Editorial Professional / `1d` Direction C — Pipeline are the four directions that were *considered*, three of them the roads not taken. The chosen direction (`2a`) is the entire redesign already delivered across §01–§40, so there is nothing to build here — and **no directions-comparison UI** was added (building rejected directions would invent UX). **`hs-s41a` re-verify (additive only):** Live tokens match Master `2a` — forest action colour (`.rd-btn--primary` / `.btn-forest` → `var(--forest)`), gold only as hairline (`--table-header-rule: var(--gold)`), body **13px Inter** (`--type-body` / `--font-ui` in `Redesign/redesign-tokens.css`), **8px spacing** (`--space-1`…`4px`/`8px`), Cormorant reserved for wordmark/display. Master `2b` night keeps brand chrome — deep forest bar `#16241E` + dimmed gold rule `#6B5738` on `dark-mode` / `data-theme="dark"`, with the same tokens aliased to `rd-scope.night` / `data-theme="night"` so page-level night selectors cannot flatten the product to greyscale. Contract documented on the token block in `css/redesign-overrides.css`. Cache-bust: `hs-s41a`.
...
- **Status:** done on `cursor/dashboard-views-017e` — verified `furniture-s42b`; the final section. See the implementation note below.
...
>
> **`furniture-s42b` Master copy pass (additive only):** `35d` Never chips now match the fixed Master list (Guest names · Addresses · Budget · Other vendors' pricing · Covenant · Notes) with live-records / honest-revocation body copy; `34d` adds the will/will-not **Downstream** statement, conflict note, “nobody edits blind” eyebrow, and one-⌘Z hint; `35b` states the consistency rule in Master wording; Day-of (`42b`) type floor raised to **11.5px** (current stays 26px, next 15px). No product additions removed. Cache-bust: `furniture-s42b` / `pat-s42b`.
