...
- **Status:** realized by the redesign on `cursor/dashboard-views-017e` — review, then say “next” for §42. No new surface: see the note below.
- **Screen ids:** `2a`, `2b`, `1a`, `1b`, `1c`, `1d`

> **Implementation note (§41).** This section is a **design-decision record**, not a shippable page — its screen headers carry no build notes, and the Master frames it as “Do not invent UX.” The full-page drawings `2a` (Neutral Records × Editorial chrome) and `2b` (its night mode) are renderings of the *shipped* house style; the view-switcher drawings `1a` Baseline / `1b` Direction A — Neutral Records / `1c` Direction B — Editorial Professional / `1d` Direction C — Pipeline are the four directions that were *considered*, three of them the roads not taken. The chosen direction (`2a`) is the entire redesign already delivered across §01–§40, so there is nothing to build here. Verified against `2a`’s stated principles in the live build: forest is the action colour and `.rd-btn--primary` fills with `var(--forest)` — **gold is only a hairline** (`--table-header-rule`), never a button fill; **body type is 13px Inter** with Cormorant reserved for display titles; the 8px spacing scale and one-primary-button-per-view hold on the shipped pages. Night theme (`2b` and the four directions in dark) reuses the same tokens, as everywhere. Building the rejected directions or a directions-comparison UI would contradict the Master’s own instruction, so none was added.
