...
(function () {
  ...
  function findTable(btn) {
    /* Prefer the redesign Tasks host (CWP mount) when present. */
    var cwpHost = document.getElementById('cwp-tasks');
    if (cwpHost) {
      var cwpTable = cwpHost.querySelector('table');
      if (cwpTable) return cwpTable;
    }
    var toolbar = btn && btn.closest ? btn.closest('.rd-toolbar') : null;
    if (!toolbar) return null;
    var scope = btn.closest('.rd-main, .rd-page, .rd-surface, .panel') || document;
