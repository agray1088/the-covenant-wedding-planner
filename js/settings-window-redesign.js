  ...
  function cloudSyncPaneBody() {
    ...
    if (!st.enabled && st.state === 'disabled') {
      ...
      html += cardRow('API base URL', 'Stored in this browser only',
        ...
        + '">');
      html += cardRow('Enable cloud sync', 'Still offline-first; guests + vendors + payments + budget + seating + contracts sync in beta',
    ...
    html += cardRow('Sync now', 'Pull then push guests + vendors + payments + budget + seating + contracts (last-write-wins)', btn('Sync now', 'rdCloudSyncNow'));
    html += cardRow('Upload this wedding', 'Create/link cloud wedding and push local guests + vendors + payments + budget + seating + contracts', btn('Upload this wedding', 'rdCloudUpload'));
    ...
    html += '<div class="rd-set__note">Honest scope: <b>guests + vendors + payments + budget + seating + contracts</b> sync in this beta. Packets, rentals, and timeline stay on-device until later passes.</div>';
    return html;
  }
  ...
  function paneHtml(id) {
    ...
    if (id === 'cloud') {
      return paneShell('Cloud sync (beta)',
        'Optional. Offline planning always works. Guests, vendors, payments, budget, seating, and contracts sync in beta — not full multi-user realtime yet.',
        cloudSyncPaneBody());
    }
    if (id === 'trash') {
    ...
    if (id === 'about') {
      return paneShell('About',
        ...
        '<div class="rd-set__note">Offline by default: no account required, no tracking. Optional <b>Cloud sync (beta)</b> can mirror guests, vendors, payments, budget, seating, and contracts to a server when you enable it — core planning never depends on being online. Look &amp; feel lives in Profile &amp; Display; this window holds backups, exports, printing, history and regional format.</div>');
    }
    if (id === 'getstarted') {
    ...
    try {
      ...
      if (name === 'rdCloudSyncNow') {
        ...
        window.CovenantCloudSync.syncNow()
          .then(function () { cloudMsg(true, 'Guests + vendors + payments + budget + seating + contracts synced (beta).'); })
      ...
      if (name === 'rdCloudUpload') {
        ...
        window.CovenantCloudSync.uploadWedding()
          .then(function () { cloudMsg(true, 'Wedding uploaded — guests + vendors + payments + budget + seating + contracts pushed.'); })
